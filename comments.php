  ...
  <ul id="comments">
   ...
   <?php
    if (comments_open()) :
     ...
     <?php } elseif ($user_ID && array_shift(array_keys($current_user->wp_capabilities)) == 'subscriber') { ?>
  	<div id="comment_login" class="messagebox">
      <p><?php printf(__('You must be affiliated with an institution to post a comment.', 'arclite'), $login_link); ?></p>
  	</div>
	 <?php } else { ?>
      ...
      <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform" onsubmit="return validatecomment(this);">
        ...
        <div class="row">
			...
			<p>To use latex in your comment, the syntax is: <code>$</code><i> code goes here </i><code>$</code>.</p>
