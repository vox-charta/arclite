...
<?php /* Arclite/digitalnature */
  global $ishome, $show_everyone, $schedaffil, $institution, $wpdb, $mobile, $page_uri, $today, $showabstracts;
...
if ($sub_is_inst) {
	...
	if (!isset($_COOKIE['schedule_affiliation'])) {
		setcookie('schedule_affiliation','Harvard-Smithsonian CfA',time()+365*24*3600,'/','.voxcharta.org');
		$schedaffil = 'Harvard-Smithsonian CfA';
	} else {
...
<title>
<?php if ($wp_query->query_vars['day'] > 0) { //Hack to fix title for 404 pages
  ...
  <div id="header-wrap">
   <div id="header" class="block-content">
     <div id="pagetitle">
	  ...
	  <div id="pagehead">
	  <h5>Astro-ph discussions for <a href="http://<?php echo $institution->subdomain; ?>.voxcharta.org/tools/institution-stats/" class="titlelink"><?php
	  ...
	  foreach ($institutions as $i => $inst) {
	  	if ($inst['name'] === $schedaffil) {
	  		echo "<option selected value='{$subdomains[$i]}'>{$inst['name']}</option>";
	  	} else {
	  		echo "<option value='{$subdomains[$i]}'>{$inst['name']}</option>";
	  	}
	  }
	  ?>
	  ...
	  <div class="disc-details">
        <p>Discussion <?php echo $institution->location; ?></p>
...
echo (is_user_logged_in()) ? '&action=logout&_wpnonce='.wp_create_nonce('log-out').'">Log out</a> | <a href="http://'.$institution->subdomain.'.voxcharta.org/wp-admin/">Site admin' : '">Log in</a> | <a href="http://'.$institution->subdomain.'.voxcharta.org/wp-login.php?action=register&affiliation='.$institution->name.'&redirect_to='.$_SERVER['REQUEST_URI'].'">Register';
echo '</a>';
?></div></div></div>
