...
function SetUser($user_ID) {
	...
	if ($id_raw != '') {
		...
		//entry does not exist
		$wpdb->query("INSERT INTO ".$wpdb->prefix."votes_users (user, votes, sinks, affiliation) VALUES(".$u_ID.", '', '', 'UCSC') ") or die(mysql_error());
	}
}
...
function GetVoteCounts($post_ID, &$count, &$evcount) {
	global $wpdb, $institution;
...
function GetVotes($post_ID, $count = '', $evcount = '') {
	global $wpdb, $institution;

	if (!isset($count) || $count == '') {
		GetVoteCounts($post_ID, $count, $evcount);
	}

	$votestr = (abs($count) == 1) ? 'vote' : 'votes';
	$evotestr = (abs($evcount) == 1) ? 'vote' : 'votes';
	return '<span style="font-size: 15pt; letter-spacing: -1px;">'.$count.' '.$votestr.' @'.$institution->name.'</span><br><span style="font-size: 8pt;">('.$evcount.' '.$evotestr.' over all institutions)</span>';
}
...
function Suggest($post_ID, $user_ID, $nice_name) {
	...
	$email_body .= get_post_meta_data($postdat) . "\n";
	$email_body .= "<p>".$postdat->post_content."</p>\n";
	$email_body .= "<br><p>If you find the paper interesting, please vote for it so we can discuss it at coffee!</p></div>\n";
...
function Vote($post_ID, $user_ID, $type, $date = null) {
	...
	$count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->users WHERE ID = '$u_ID'"));
	if ($count == 0) return;
...
function Unvote($post_ID, $user_ID) {
	...
	//Gets the votes
	$votes_raw = $wpdb->get_row("SELECT votes, votetimes, usersinks AS sinks, sinktimes FROM  ".$wpdb->prefix."votes WHERE post='{$p_ID}' AND institution='{$institution->ID}'");
	$uservotes_raw = $wpdb->get_row("SELECT votes, sinks FROM ".$wpdb->prefix."votes_users WHERE user='".$u_ID."'");
...
function AgendaOffset($type = 'prev', $stop_for = 'an', $agenda_time = null) {
	...
	global $today, $agenda_info, $institution, $wpdb, $yesdays, $nodays,
	...
	$reset_time = (!isset($institution)) ? get_option('voteiu_reset_time') : $reset_time = GetResetTime($agenda_time);
		...
		if (count($events) != 0) {
			...
			foreach ($events as $e => $event) {
				if ($event->extradays != '') {
					...
					sort($yesevent);
					$yeseventdays[$e] = array(count($yesevent));
					foreach($yesevent as $i => $except) {
						$yeseventdays[$e][$i] = $dayssign*round(($agenda_time - GetResetTime(strtotime($except)))/86400);
					}
				}
						...
						if (count($rangearr) == 2) {
							$noeventdays[$e] = array_merge($noeventdays[$e],
								range($dayssign*round(($agenda_time - GetResetTime(strtotime($rangearr[0])))/86400),
									  $dayssign*round(($agenda_time - GetResetTime(strtotime($rangearr[1])))/86400), 7));
	...
	while (1) {
		...
		foreach($events as $e => $event) {
			if ($event->dayofweek != '' && $offset_date == $event->dayofweek) { 
...
function InstitutionEdit($institution_ID) {
	...
	if (array_key_exists('name', $_POST)) {
		...
		$wpdb->query("UPDATE {$wpdb->prefix}votes_institutions SET name = '{$_POST['name']}'  WHERE ID = '{$i_ID}'");
		$wpdb->query("UPDATE {$wpdb->prefix}votes_users SET affiliation = '{$_POST['name']}'  WHERE affiliation = '{$old_name}'");
	}
	...
	if (array_key_exists('closedelay', $_POST)) {
		...
		if (!is_numeric($_POST['closedelay'])) {
			echo "<font color='red'><h3>Error: {$_POST['closedelay']} is an invalid time!</h3></font>";
			$failed = true;
		}
	...
	if (array_key_exists('announcedelay', $_POST)) {
		...
		foreach ($delays as $i => $delay) {
			...
			if (!is_numeric($delays[$i])) {
				echo "<font color='red'><h3>Error: {$_POST['announcedelay']} is an invalid duration!</h3></font>";
				$failed = true;
			}
		}
		if (!$failed) {
	...
	if (!empty($_POST)) {
		$cronstr =
			"31,36,41,46,51,56 17 * * 0-4 /usr/bin/curl http://voxcharta.org/wp-content/plugins/wp-o-matic/cron.php?code=7175e58b\n" .
			"1,6,11,16,21,26,31,41,51 18 * * 0-4 /usr/bin/curl http://voxcharta.org/wp-content/plugins/wp-o-matic/cron.php?code=7175e58b\n" .
			"1,11,21,31,41,51 19-20 * * 0-4 /usr/bin/curl http://voxcharta.org/wp-content/plugins/wp-o-matic/cron.php?code=7175e58b\n" .
			...
			//"1,11,21,31,41,51 19-23 * * 0-4 /usr/bin/curl http://voxcharta.org/wp-content/plugins/wp-o-matic/cron.php?code=7175e58b\n". //Emergency cron line
			//"1,11,21,31,41,51 0-16 * * 0-4 /usr/bin/curl http://voxcharta.org/wp-content/plugins/wp-o-matic/cron.php?code=7175e58b\n". //Emergency cron line
		$institutions = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}votes_institutions WHERE name != 'Unaffiliated' AND active = 1");
		
		date_default_timezone_set('US/Pacific');
		$rectime = date("i G", strtotime('6:00 pm'));
		...
		foreach ($institutions as $inst) {
			...
			$cronstr .= $rectime." * * 0-4 /usr/bin/curl http://voxcharta.org/wp-content/plugins/vote-it-up/recommend-cron.php?code=7175e58b\&institution=".urlencode($inst->name)."\n";
		}
...
function UserPosts($u_ID) {
	...
	//Get posts logged, and get the latest posts voted [new posts have larger IDs
	$posts = $wpdb->get_results("SELECT post, votes, usersinks, lastvotetime FROM ".$wpdb->prefix."votes WHERE ID != '' AND lastvotetime > 0 ORDER BY lastvotetime DESC");
	foreach ($posts as $post) { 
...
function AuthorSort() {
	...
	$authors = $wpdb->get_results("SELECT t.term_id AS id, t.name AS name, t.slug AS slug, ".
		//"GROUP_CONCAT(if (va.affiliations ='', null, va.affiliations) SEPARATOR '|') AS affiliations, ".
		...
		//"LEFT JOIN {$wpdb->prefix}votes_authors AS va ON (t.name = va.aliases) ".
	...
	//	$voteinfo = $wpdb->get_results("SELECT p.ID AS pid, vo.votes AS votes, vo.usersinks AS sinks FROM {$wpdb->posts} AS p
	//		INNER JOIN {$wpdb->prefix}votes AS vo ON (p.ID = vo.post)
	//		INNER JOIN {$wpdb->term_relationships} AS tr ON (p.ID = tr.object_id)
	//		INNER JOIN {$wpdb->term_taxonomy} AS tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id)
	//		INNER JOIN {$wpdb->terms} AS t ON (tt.term_id = t.term_id)
	//		WHERE t.term_id = '{$author->id}'
	//		AND tt.taxonomy = 'post_author'
	//		AND (vo.votes != '' || vo.usersinks != '');");
		...
		//		FROM wp_votes_authors WHERE affiliations != '' AND LOWER(aliases) LIKE '%".strtolower($author->name)."%'");
		//$max_dates = explode(',',$affiliations->affilmaxdates);
		//$max_utime = array();
		//foreach ($max_dates as $max_date) {
		//	$max_utime[] = strtotime($max_date);
		//}
		//$affil_arr = explode('|',$affiliations->affiliations);
		//$affilsarray[] = $affil_arr[array_search(max($max_utime), $max_utime)];

		$namearray[] = $author->name;
		$affilsarray[] = $author->affiliations;
	...
	array_multisort($sortarray, $order, $sorttype, $countarray, SORT_DESC,
					$votecountarray, SORT_DESC, $sinkcountarray, $namearray, $slugarray, $affilsarray);

	$output = array('count' => $countarray, 'name' => $namearray, 'slug' => $slugarray,
					'votecount' => $votecountarray, 'sinkcount' => $sinkcountarray, 'affil' => $affilsarray);
...
function SortVotes($agenda_time = 0, $type = '', $force_everyone = false, $post_ID = null) {
	...
	if ($post_ID) {
		...
		$disc_time = GetDiscussionTime($agenda_time);
	...
	$posts = $wpdb->get_results("SELECT post, votes, usersinks, votetimes, sinktimes, lastvotetime, guests, guestsinks, guestvotetimes, guestsinktimes, discussed, institution FROM ".$wpdb->prefix.
		"votes WHERE ID != ''{$inst_query} ORDER BY lastvotetime DESC");
	...
	*/
	...
	foreach ($posts as &$post) { 
		...
		if ($post->votes != '') {
			...
			foreach ($votetimes_raw as $i => $votetime) {
				if (($ishome && $institution->pushnotdiscussed && !$post->discussed && $post->institution == $institution->ID) ||
			...
			array_multisort($votes_new, $votefutures, $votetimes_new);
		...
		if ($post->usersinks != '') {
			...
			array_multisort($sinks_new, $sinktimes_new, $sinkfutures);
		...
		$first_vote_time = (count($votetimes_new) + count($sinktimes_new) != 0) ? min(array_merge($votetimes_new, $sinktimes_new)) : 0;
	...
	//} else {
		array_multisort($discussedarray, SORT_DESC, $futurearray, SORT_ASC, $sumarray, SORT_DESC,
			...
			$votefuturesarray, $sinkfuturesarray);
	//}
	...
	foreach ($output['votetimes'] as $i => $votetimes) {
		...
		array_multisort($output['votetimes'][$i], SORT_ASC, $output['votenames'][$i], $output['voteinsts'][$i], $output['votefutures'][$i]);
	...
	foreach ($output['sinktimes'] as $i => $sinktimes) {
		...
		array_multisort($output['sinktimes'][$i], SORT_ASC, $output['sinknames'][$i], $output['sinkinsts'][$i], $output['sinkfutures'][$i]);
	}
	return $output;
}
...
function MostVotedAllTime_Cron($a = '', $incl_abstracts = false, $incl_meta = false, $showing_everyone = false) {
    ...
    while ($rows < get_option('voteiu_widgetcount') && (!$showing_everyone || $rows < 5)) {
    	if ($a['pid'][$i][0] != '') {
    		...
    		if (!empty($postdat)) {
				...
				if ($incl_meta) {
					...
					$email_str .= get_post_meta_data($postdat) . "\n";
				}
				if ($incl_abstracts) {
...
function GetResetTime($time) {
	global $institution, $grt_discussiondays, $grt_resettimes;

	if (!isset($grt_discussiondays) || !isset($grt_resettimes)) {
	...
	return strtotime($grt_resettimes[0].$mdy);
}

function GetDiscussionTime($time) {
	global $institution;
	...
	return strtotime($resettimes[0].date(" m/d/Y", $time));
}
